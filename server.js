...
app.post('/mix', async (req, res) => {
  ...
  try {
    ...
    const ffmpegCommand = `
      ffmpeg -y -i "${videoPath}" -i "${musicPath}" -filter_complex "[0:v]copy[vout];[1:a]volume=0.25[a1];[0:a][a1]amix=inputs=2:duration=first[aout]" -map "[vout]" -map "[aout]" -c:v copy -t $(ffprobe -v quiet -show_entries format=duration -of csv=p=0 "${videoPath}") "${outputPath}"
    `;
