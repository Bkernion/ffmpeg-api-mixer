...
app.post('/mix', async (req, res) => {
  ...
  try {
    ...
    const ffmpegCommand = `
      ffmpeg -y -i "${videoPath}" -i "${musicPath}" -filter_complex "[1:a]volume=0.25,aloop=loop=-1:size=2e+09[a1];[0:a][a1]amix=inputs=2:duration=first[aout]" -map 0:v -map "[aout]" -c:v copy -shortest "${outputPath}"
    `;
